    ...
    async def poll_alarm(self):
        ...
        for sensor in self.sensor_manager.sensors:
            ...
            if event_duration >= float(Config.get().alarmDuration * 60):
                ...
                await self.send_alarm_event(sensor.zone, sensor_ctx.occupied_start_time, event_duration)
                # Set the alarm sent flag to True
                sensor_ctx.alarm_sent = True

    async def send_alarm_event(self, zone: str, start_time: datetime, duration: float) -> None:
        # Create the alarm event
        alarm_event = EventData.alarm_event(zone, start_time, duration)
